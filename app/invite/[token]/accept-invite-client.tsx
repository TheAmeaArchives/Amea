...
export default function AcceptInviteClient({
  ...
  async function acceptInvite() {
    ...
    try {
      const response = await fetch(`/api/public/admin-invites/${encodeURIComponent(token)}/accept`, {
        method: "POST",
        credentials: "include",
      });
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-xl items-center justify-center px-4 py-10">
      <div className="w-full rounded-2xl border bg-white p-8 shadow-sm">
        <p className="text-sm font-medium text-default">Admin Invitation</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Join AMEA as {invite.role === "super_admin" ? "a Super Admin" : "an Admin"}
        </h1>
        <p className="mt-3 text-sm text-muted-foreground">
          This invite was sent to <span className="font-medium text-foreground">{invitedEmail}</span>.
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          Expires on {new Date(invite.expires_at).toLocaleString()}.
        </p>

        {!isInviteUsable ? (
          <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            This invite is {invite.status}. Ask a super admin to send a new one if you still need access.
          </div>
        ) : null}
            ...
            {checkingSession ? (
              ...
              <>
                <p className="text-sm text-muted-foreground">
                  Signed in as the invited user. You can accept the invite now.
                </p>
              ...
              <>
                <div className="space-y-2">
                  ...
                  <Input id="invite-email" value={invitedEmail} disabled />
                </div>
