...
const Search = ({ mode }: { mode: "ICON" | "BAR" }) => {
  ...
  return (
    <div className="flex-1 h-fit flex justify-end">
      ...
      {mode === "BAR" && (
        <div className="h-10 border w-full max-w-4xl mx-auto rounded-full overflow-hidden">
          <input
            type="text"
            placeholder="Search..."
            className="duration-300 transition-all  w-full outline-none text-black focus:outline-none focus-within:outline-none pr-11 pl-4 h-full border-none"
          />
        </div>
      )}
    </div>
